...
## We got rid of the random slope and used the random intercept only model to avoid convergence issue.
...
ggplot(pred_df, aes(x = year, y = emmean,
  ...
  # 🔧 Automatic subsetting fix: ensures only levels present are used
  scale_color_manual(
    values = custom_colors[names(custom_colors) %in% unique(pred_df$Group_Level)]
  ) +
  scale_fill_manual(
    values = custom_colors[names(custom_colors) %in% unique(pred_df$Group_Level)]
  ) +
  scale_linetype_manual(
    values = custom_linetypes[names(custom_linetypes) %in% unique(pred_df$Group_Level)]
  ) +
  guides(
    fill = "none",
    color = guide_legend(
      override.aes = list(
        linewidth = 1.0,              
        linetype  = custom_linetypes[names(custom_linetypes) %in% unique(pred_df$Group_Level)],
        color     = custom_colors[names(custom_colors) %in% unique(pred_df$Group_Level)]
      )
    )
  ) +
  # 人工畫軸線 (交會點 0, -10)
...
# sapply(model_data[, c("HbA1c_G4", "PTGENDER_all", "PTMARRY_all", 
#                       "AAPOEGNPRSNFLG_all", "PTETHNIC_all")],
#        function(x) length(unique(x)))

## we know that PTEthnic only has one level so we have to remove it from the model
PACC_HbA1c_spline <- lm(
  PACC.raw ~ ns(year, df = 3) * HbA1c_G4 +
    PTAGE_all + PTGENDER_all + PTMARRY_all +
...
# Make sure pred_df has actual numeric values
pred_df <- pred_df %>% 
  filter(!is.na(emmean))  # remove rows with NA predictions

# Compute axis limits safely
x_max <- ifelse(all(is.na(pred_df$year)), 1, max(pred_df$year, na.rm = TRUE))
y_min <- ifelse(all(is.na(pred_df$lcl)), 0, min(pred_df$lcl, na.rm = TRUE))
y_max <- ifelse(all(is.na(pred_df$ucl)), 1, ceiling(max(pred_df$ucl, na.rm = TRUE)))
...
ggplot(pred_df, aes(x = year, y = emmean,
  ...
  geom_ribbon(aes(ymin = lcl, ymax = ucl, fill = Group_Level),
              alpha = 0.18, color = NA, show.legend = FALSE) +
  geom_line(linewidth = 1.2, na.rm = TRUE) +  # skip NAs safely
  labs(
    title = "Modeled PACC",
    x = "Year", y = "Modeled mean PACC"
  ) +
  ...
  # Draw axes manually
  geom_segment(x = 0, xend = x_max, y = y_min, yend = y_min, color = "black") +
  geom_segment(x = 0, xend = 0, y = y_min, yend = y_max, color = "black") +
  scale_x_continuous(limits = c(0, NA)) +
...
# model_data <- na.omit(PACC_all[, c("PACC.raw", "year", "chol200_G4", 
#                                    "PTAGE_all", "PTGENDER_all", "PTMARRY_all",
#                                    "AAPOEGNPRSNFLG_all", "PTETHNIC_all")])

# sapply(model_data[, c("chol200_G4", "PTGENDER_all", "PTMARRY_all", 
#                       "AAPOEGNPRSNFLG_all", "PTETHNIC_all")],
#        function(x) length(unique(x)))
